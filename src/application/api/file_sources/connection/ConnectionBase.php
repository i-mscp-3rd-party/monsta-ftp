<?php
    ...
    abstract class ConnectionBase {
        ...
        public function listDirectory($path, $showHidden = null) {
            $this->ensureConnectedAndAuthenticated('list directory');
        ...
        public function downloadFile($transferOperation) {
            $this->ensureConnectedAndAuthenticated('download');

            if (!$this->handleDownloadFile($transferOperation))
                $this->handleOperationError('download', $transferOperation->getLocalPath(), error_get_last(),
        ...
        public function uploadFile($transferOperation) {
            $this->ensureConnectedAndAuthenticated('upload');

            if (!$this->handleUploadFile($transferOperation))
                $this->handleOperationError('upload', $transferOperation->getLocalPath(), error_get_last(),
                    $transferOperation->getRemotePath());
        }
        ...
        public function deleteFile($remotePath) {
            $this->ensureConnectedAndAuthenticated('delete file');

            if (!$this->handleDeleteFile($remotePath))
                $this->handleOperationError('delete file', $remotePath, error_get_last());
        }
        ...
        public function makeDirectory($remotePath) {
            $this->ensureConnectedAndAuthenticated('make directory');

            if (!$this->handleMakeDirectory($remotePath))
                $this->handleOperationError('make directory', $remotePath, error_get_last());
        }
        ...
        public function deleteDirectory($remotePath) {
            $this->ensureConnectedAndAuthenticated('delete directory');
            ...
            if (!$this->handleDeleteDirectory($remotePath))
                $this->handleOperationError('delete directory', $remotePath, error_get_last());
        }

        public function deleteMultiple($remotePathsAndTypes) {
            $this->ensureConnectedAndAuthenticated('delete multiple');
        ...
        public function rename($source, $destination) {
            $this->ensureConnectedAndAuthenticated('rename');

            if (!$this->handleRename($source, $destination))
                $this->handleOperationError('rename', $source, error_get_last(), $destination);
        }
        ...
        public function changePermissions($mode, $remotePath) {
            $this->ensureConnectedAndAuthenticated('change permissions');

            Validation::validatePermissionMask($mode, false);

            if (!$this->handleChangePermissions($mode, $remotePath))
                $this->handleOperationError('change permissions', $remotePath, error_get_last());
        }
        ...
        public function copy($source, $destination) {
            $this->ensureConnectedAndAuthenticated('copy');
